...
for N in Ns:
    for H_p in H_ps:
        ...
        for T_p in T_ps:
            if H_p == 0:
                if 2.1 <= T_p <= 2.5:
                    ...
                    E_p, M, S_new = Solve2DIsing(S, T_p, steps[2], H_p)
                else:
        ...
        if H_p == 0:
            np.save(f'Data/E_ps_N{N}_H_p{H_p}_steps{steps[0]}-{steps[1]}-{steps[2]}.npy', E_ps)
            np.save(f'Data/Ms_N{N}_H_p{H_p}_steps{steps[0]}-{steps[1]}-{steps[2]}.npy', Ms)
            np.save(f'Data/M_norms_N{N}_H_p{H_p}_steps{steps[0]}-{steps[1]}-{steps[2]}.npy', M_norms)
        else: 
            np.save(f'Data/E_ps_N{N}_H_p{H_p}_steps{Hsteps}.npy', E_ps)
            np.save(f'Data/Ms_N{N}_H_p{H_p}_steps{Hsteps}.npy', Ms)
            np.save(f'Data/M_norms_N{N}_H_p{H_p}_steps{Hsteps}.npy', M_norms)
