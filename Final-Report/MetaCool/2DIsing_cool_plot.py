...
for N in Ns:
    ...
    for H_p in H_ps:
        if H_p == 0:
            E_ps = np.load(f'Data/cool_E_ps_N{N}_H_p{H_p}_steps{steps[0]}-{steps[1]}.npy')
        else:
            E_ps = np.load(f'Data/cool_E_ps_N{N}_H_p{H_p}_steps{Hsteps}.npy')
    ...
    # ax.invert_xaxis()
    fig.tight_layout()
    plt.savefig(f'Figs/cool_energy_N{N}_steps{steps[0]}-{steps[1]}_Hsteps{Hsteps}.png', bbox_inches='tight')
    plt.show()
...
for N in Ns:
    ...
    for H_p in H_ps:
        if H_p == 0:
            M_norms = np.load(f'Data/cool_M_norms_N{N}_H_p{H_p}_steps{steps[0]}-{steps[1]}.npy')
        else:
            M_norms = np.load(f'Data/cool_M_norms_N{N}_H_p{H_p}_steps{Hsteps}.npy')
    ...
    # ax.invert_xaxis()
    fig.tight_layout()
    plt.savefig(f'Figs/cool_magnetization_N{N}_steps{steps[0]}-{steps[1]}_Hsteps{Hsteps}.png', bbox_inches='tight')
    plt.show()
